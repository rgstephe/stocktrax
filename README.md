...
it arrives, scan it out when someone takes it, and every movement is logged to the
...
This is the **Community Edition**: free to self-host, no build step, one
Docker container. A hosted subscription version of Allokis is also offered by
Ultra Pest Control.
...
- **Receive stock by barcode.** Scan a new product and Allokis looks it up in
...
- **White-labeling.** Set your company name, tagline, and logo in Settings and
  Allokis rebrands itself across the kiosk, landing page, and browser tab.
- **Full audit log.** Who took what, and when: receives, checkouts, returns,
...
```bash
git clone <your-repo-url> allokis
cd allokis
...
Your database persists in `./data/allokis.db`. (Installs from before the
rename have `stocktrax.db`; it is renamed automatically on first start.)
...
# ("stocktrax" is the service name in docker-compose.yml; use yours if you renamed it)
```
...
automatically and turns your existing admin PIN into your new password. Log
in with `Admin` and your old PIN, then change it.

**Network exposure:** Allokis is built to run on your own trusted network,
...
In plain terms: you're free to use, modify, and self-host Allokis. If you
...
This repository is the **Community Edition**. Ultra Pest Control also offers
Allokis as a paid, hosted subscription service, which builds on this same core.
As the copyright holder, Ultra Pest Control may offer the code under other
license terms as well.

Outside contributions are welcome and require agreeing to the
[Contributor License Agreement](CLA.md). See [CONTRIBUTING.md](CONTRIBUTING.md).

## Trademark

"Allokis" and the Allokis logo are trademarks of Ultra Pest Control. The AGPL
covers the code only; it does not grant permission to use the name or logo.

- You may say your software is "based on Allokis" or "a fork of Allokis."
- If you distribute a modified version or run it as a service for others, give
  it a different name and remove the Allokis logo (the files in `public/img/`
  and the default logo in the page headers).
- Self-hosting unmodified Allokis for your own business is fine as is.
